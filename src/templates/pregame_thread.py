...
class PreGameThread:
    ...
    def build_title(self, game: Game) -> str:
        return f'PRE GAME THREAD: {self.__get_title(game)}'

    def build_body(self, game: Game) -> str:
        template = Template(filename=self.__get_file_path('pregame_thread.md'))
        context = {
            'header': self.__get_header(game),
            'conditions': self.__get_conditions(game),
            'starting_lineups': self.__get_lineups(game),
            'broadcast': self.__get_broadcast(game),
